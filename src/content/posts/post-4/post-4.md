...
# 信号发生器
![信号发生器](image-9.png)
DAC是STM32输出的波形，比如正弦波，三角波等，DAC_OFFSET是一个直流电压，控制波形的偏置  
DAC和DAC_OFFSET经电压跟随器提升带载能力  
>电压跟随器  
可提高输入阻抗(若后级阻抗较低，就会从DAC取电流，DAC输出电压可能发生变化)，它的输入阻抗非常大，DAC不需要输出电流，So DAC的电压不会被拉低    
降低输出阻抗(后级所需电流过大，DAC带不动)，加电压跟随器后，真正提供电流的是运放  
隔离前后级，前级和后级不会相互影响
>
C81 C85为高频滤波  
R91和R92分压得  
$$
V_{+}=V_{DAC}×\frac{R_{92}}{R_{91}+R_{92}}=0.75V_{DAC}
$$
$R_{88}$与U55.4构成负反馈，由于虚短
$$
V_{-}=V_{+}=0.75V_{DAC}
$$
由KCL得
$$
\frac{V_{-}-V_{DACOFFSET}}{R_{87}}+\frac{V_{OUT}-V_{-}}{R_{88}}=0
$$
$$
V_{OUT}=3(V_{-}-V_{DACOFFSET})
$$
故U55.4为差分放大器，将同向端电压-反向端电压放大3倍输出  
